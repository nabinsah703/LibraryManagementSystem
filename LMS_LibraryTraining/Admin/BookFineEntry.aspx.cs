...
namespace LMS_LibraryTraining.Admin
{
    public partial class BookFineEntry : System.Web.UI.Page
    {
        ...
        {
            cmd = new SqlCommand("sp_getMember_ByID", conn.GetConnection());
        ...
        protected void btnsubmit_Click(object sender, EventArgs e)
        {
            ...
            {
                Response.Write("<script>alert('Error','wrong member ID ...try again')</script>");
            }
        }

        private void InsertBookFine()
        {
            cmd = new SqlCommand("sp_InsertFineDetials", conn.GetConnection());
            ...
            {
                ReturnBook();
                Response.Write("<script>alert('Success','Books Fine Paid and Book Return Successfully')</script>");
            }
            else
            {
